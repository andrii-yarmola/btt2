...
function initLayoutFix(){
	...
	if (isTouchDevice) {
		...
		var hold = jQuery('.win-height');

		function handleHeight(){
			hold.css({maxHeight: ''});
			setTimeout(function(){
				hold.css({maxHeight: hold.outerHeight()});
			}, 200);
		}

		handleHeight();
		win.on('orientationchange', handleHeight);
	}
}
...
function initTabLinks(){
	...
	jQuery('.tab-link').on('click', function(e){
		e.preventDefault();
		var navInstance = body.data('MobileNav');
		var animSpeed = jQuery(window).scrollTop() > 0 ? 650 : 0;
		jQuery('html, body').stop().animate({scrollTop: 0}, animSpeed, function(){
			if (navInstance && !navInstance.isOpened()) {
				navInstance.show();
			}
		});
		jQuery('.nav-tabs a[href="'+ this.getAttribute('href') +'"]').tab('show');
	});
}

// init textarea autosize
function initTextarea(){
	jQuery('.contact-form textarea').textareaAutoSize();
}
...
function initPromo(){
	ResponsiveHelper.addRange({
		'..767': {
			on: function() {
				jQuery('.history-block').fadeGallery({
					slides: 'div.step',
			...
			off: function() {
				if (jQuery('.history-block').data('FadeGallery')) {
					jQuery('.history-block').data('FadeGallery').destroy();
				}
			}
		},
		'768..': {
			on: function() {
				jQuery('.history-block').promo({
					box: '.container',
					slides: '[data-num]'
				});
			}
		...
		onInit: function(self){
			ResponsiveHelper.addRange({
				'768..': {
					on: function() {
						var maxHeight = 0;
						self.innerMask = self.holder.find('.inner-gallery .scroll-mask')
						self.innerSlides = self.innerMask.find('.scroll-slide');
						self.handleHeight = function(){
							maxHeight = 0;
							self.innerSlides.each(function(){
								if (jQuery(this).outerHeight() > maxHeight) {
									maxHeight = jQuery(this).outerHeight();
								}
							})
							self.innerMask.css({height: maxHeight});
						}
						self.handleHeight();
						self.bindHandlers(['handleHeight']);
						self.win.on('resize orientationchange', self.handleHeight);
						setTimeout(function(){
							self.handleHeight();
						}, 1000);
					},
					off: function() {
						self.win.off('resize orientationchange', self.handleHeight);
						setTimeout(function(){
							self.innerMask.removeAttr('style');
						}, 0);
					}
				}
			});
...
;(function($){
	function Promo(options){
		this.options = $.extend({
			fixedClass: 'fixed-mode',
			activeClass: 'active',
			bottomClass: 'bottom-mode'
		},options);
		this.init();
	}
	Promo.prototype = {
		...
		findElements: function(){
			...
			this.slides.each(function(){
				var slide = $(this);
				var max = Math.max(slide.data('num').toString().split(','));
			...
			ResponsiveHelper.addRange({
				'768..': {
					on: function() {
						self.resizeHandler();
						self.scrollHandler();
						self.bindHandlers(['resizeHandler']);
						self.bindHandlers(['scrollHandler']);
						self.win.on('resize load orientationchange', self.resizeHandler);
						self.win.on('scroll load orientationchange', self.scrollHandler);
						setTimeout(function(){
							self.resizeHandler();
							self.scrollHandler();
						}, 500);
		...
		scrollHandler: function(){
			...
			if (this.config.holder.offset <= winTop && this.config.holder.offset + this.config.holder.height > winTop + this.config.win.height) {
				this.holder.addClass(this.options.fixedClass);
			} else {
				this.holder.removeClass(this.options.fixedClass);
			}

			if (winTop + this.config.win.height > this.config.holder.offset + this.config.holder.height) {
				this.holder.addClass(this.options.bottomClass);
			} else {
				this.holder.removeClass(this.options.bottomClass);
			}
			this.slides.removeClass(this.options.activeClass);
			this.config.offsets.forEach(function(offset, i) {
				if (winTop < offset && winTop + self.config.win.height > offset) {
					self.getSlides(i).addClass(self.options.activeClass);
				} else if (winTop < offset && winTop + self.config.win.height < self.config.offsets[0]) {
					self.getSlides(0).addClass(self.options.activeClass);
				} else if (winTop + self.config.win.height > self.config.holder.offset + self.config.holder.height){
					self.getSlides(self.config.max).addClass(self.options.activeClass);
				}
			});
		},
		getSlides: function(i){
			return this.slides.filter(function(index, slide){
				return slide.getAttribute('data-num').indexOf(i) !== -1
			});
		},
		resizeHandler: function(){
			var self = this;
			this.config.win.height = this.win.height();
			this.config.holder.height = this.holder.height();
			this.config.holder.offset = this.holder.offset().top;
			this.holder.css({height: this.config.win.height * this.slides.length});
			this.config.offsets = [];
			$.each(this.slides, function(i){
				self.config.offsets.push((self.config.holder.offset + self.config.win.height * (i+1)).toFixed());
			});
			this.config.offsets.sort(function(a, b){return a - b;});
		},
		...
		destroy: function(){
			var self = this;
			this.win.off('resize load orientationchange', this.resizeHandler);
			this.win.off('resize scroll load orientationchange', this.scrollHandler);
			setTimeout(function(){
				self.holder.removeClass(self.options.fixedClass);
				self.holder.removeClass(self.options.bottomClass);
				self.box.add(self.holder).removeAttr('style');
			}, 0);
			this.makeCallback('onDestroy', this);
		}
	};
	$.fn.promo = function(opt){
		return this.each(function(){
			$(this).data('Promo', new Promo($.extend(opt, {holder: this})));
		});
	};
}(jQuery));
...
function initContactBox(){
	jQuery('body').mobileNav({
		...
		skipBlocks: '.clockpicker-popover, .ui-datepicker, .ui-datepicker-header, .tab-link',
		menuOpener: '.contact-ico, .contact-box .btn-close',
		menuDrop: '.contact-box'
	});
}

// add classes on hover/touch
function initCustomHover() {
	jQuery('.works-block .work-box').touchHover();
}

// same height
function initSameHeight() {
	jQuery('.company-numbers-cols').sameHeight({
		elements: '.number-box',
		flexible: true,
		multiLine: true
	});
	jQuery('.main-statistics-cols, .project-description').sameHeight({
		elements: '.col',
		flexible: true,
		multiLine: true
	});
...
function initFileUpload(){
	...
	jQuery('.file-upload').each(function(){
		...
		var input = hold.find('input:file');
		var fileList = hold.find('.file-list');
		var fileItem = fileList.find('>li').clone();
		...
		input.fileupload({
			...
			add: function(e, data){
				...
				if (data.files[0].size > maxFileSize) {
					alert('File "'+data.files[0].name+'" is too large. Max size ~25MB.');
				}
				if (curFiles < maxFiles && data.files[0].size < maxFileSize) {
					var item = fileItem.clone();
					var $itemFile = $('<input/>').attr({
						type: 'hidden',
						name: 'attachments[]'
					}).appendTo(item);
					...
					data.context = item.appendTo(fileList);
					item.find('.name').html(data.files[0].name);

					// cancel loading item
					item.find('.cancel').on('click', function(e){
						e.preventDefault();
					...
					jqXHR = data.submit().success(function(response, textStatus, jqXHR){
						if (textStatus === 'success') {
							item.removeClass('uploading');
							$itemFile.val(response.path);
							// remove loaded item
							item.find('.remove').on('click', function(e){
								e.preventDefault();
								jQuery.ajax({
									url: response.deleteUrl,
									method: response.deleteType
								}).success(function(){
									item.fadeOut(function(){
										item.remove();
										curFiles--;
	...
	MobileNav.prototype = {
		...
		attachEvents: function() {
			...
			this.outsideClickHandler = function(e) {
				if(self.isOpened()) {
					var target = $(e.target);
					if(!target.closest(self.opener).length && !target.closest(self.drop).length && !target.closest(self.options.skipBlocks).length) {
						self.hide();
					}
				}
			};
...
;(function(){
	...
	var tpl = [
		'<div class="popover clockpicker-popover">',
			'<a href="#" class="clockpicker-close">close</a>',
			...
			'<div class="popover-content">',
				...
				'<div class="btn-holder">',
					'<button type="button" class="clockpicker-cancel">Cancel</button>',
					'<button type="button" class="clockpicker-ok btn btn-primary">Ok</button>',
				'</div>',
			'</div>',
		'</div>'
	].join('');
	...
	function ClockPicker(element, options) {
		...
		this.btnClose = popover.find('.clockpicker-close');
		this.btnCancel = popover.find('.clockpicker-cancel');
		this.btnOk = popover.find('.clockpicker-ok');
		this.spanHours = popover.find('.clockpicker-span-hours');
		this.spanMinutes = popover.find('.clockpicker-span-minutes');
		this.spanAmPm = popover.find('.clockpicker-span-am-pm');
		this.amOrPm = now.getHours() > 11 ? "PM" : "AM";
		...
		this.btnCancel.on('click', function(e){
			...
			if (self.currentView === 'hours') {
				self.hide();
			} else if (self.currentView === 'minutes') {
		...
		if (options.twelvehour) {
			...
			this.spanAmPm.html(self.amOrPm);

			$('<button type="button" class="btn btn-sm btn-default clockpicker-button am-button '+ (this.amOrPm === "AM" ? 'active' : '')  +'">' + "AM" + '</button>')
				.on("click", function() {
			...
			$('<button type="button" class="btn btn-sm btn-default clockpicker-button pm-button '+ (this.amOrPm === "PM" ? 'active' : '')  +'">' + "PM" + '</button>')
				.on("click", function() {
	...
	ClockPicker.prototype.locate = function(){
		...
		popover.show();
		// Place the popover
		if ($win.scrollTop() + $win.height() < offset.top + popover.outerHeight()) {
			placement = 'top';
		}
		switch (placement) {
			case 'bottom':
				styles.top = offset.top + height - $win.scrollTop();
			...
			case 'right':
				...
				break;
			case 'top':
				styles.top = offset.top - popover.outerHeight() - $win.scrollTop();
				styles.position = 'fixed';
				break;
			case 'left':
	...
	ClockPicker.prototype.setHand = function(x, y, roundBy5, dragging){
		...
		this[isHours ? 'spanHours' : 'spanMinutes'].html(isHours ? value : leadingZero(value));
		...
		var cx = (Math.sin(radian) * radius) * 0.7,
			cy = (- Math.cos(radian) * radius) * 0.7;
...
;(function($){
	...
	FadeGallery.prototype = {
		...
		findElements: function() {
			...
			if (this.options.autoParentHeight) {
				this.slidesHolder = this.slidesHolder.add(this.slidesHolder.parent())
			}
		...
		switchSlide: function(fast) {
			...
			if(this.slides.length > 1) {
				...
				if(!this.options.animSpeed) {
					...
					this.slides.eq(this.prevIndex).stop().animate({opacity:0},{duration: fast ? 0 : this.options.animSpeed});
				}
				...
				this.switchNext = function() {
					if(!self.options.animSpeed) {
						...
						self.slides.eq(self.currentIndex).stop().animate({opacity:1},{duration: fast ? 0 : self.options.animSpeed});
					}
		...
		destroy: function() {
			...
			setTimeout(function(){
				self.slidesHolder.add(self.slides).stop().removeAttr('style');
				if (self.options.autoParentHeight) {
					self.slidesHolder.parent().stop().removeAttr('style');
				}
			}, 0);
		}
	};
...
// init textarea autosize
function initTextarea(){
	jQuery('.payment-form textarea').textareaAutoSize();
}
